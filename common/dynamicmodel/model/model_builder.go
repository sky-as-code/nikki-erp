...
// Metadata attaches arbitrary module-defined data to the field. The engine never interprets
// it; a consuming module reads its own keys back through ModelField.Metadata(), and the
// frontend receives it in the meta/schema payload.
//
// Calling it more than once merges: later keys win, earlier ones survive. That is also what
// makes it composable with Extend — a schema extending a mixin can add a key without
// discarding the ones the mixin declared.
//
// Values must be plain JSON-serializable data. The whole schema is round-tripped through JSON
// by contract, so a func, channel or struct here would be silently lost or would fail to
// encode.
func (this *FieldBuilder) Metadata(metadata map[string]any) *FieldBuilder {
	if len(metadata) == 0 {
		return this
	}
	if this.field.metadata == nil {
		this.field.metadata = make(map[string]any, len(metadata))
	}
	for key, val := range metadata {
		this.field.metadata[key] = val
	}
	return this
}
