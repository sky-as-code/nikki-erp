...
func (this ModelSchema) Columns() []*ModelField {
	...
	for _, name := range this.fieldsOrder {
		if f, ok := this.fields[name]; ok && f != nil && !f.IsVirtual() {
...
func (this ModelSchema) ReadableFields() []*ModelField {
	result := make([]*ModelField, 0, len(this.fieldsOrder))
	for _, name := range this.fieldsOrder {
		if f, ok := this.fields[name]; ok && f != nil && !f.IsEdgeModel() {
			result = append(result, f)
		}
	}
	return result
...
// isAutoGeneratedNonKey indicates a field the server fills on write and the client may not set,
// which Validate drops from the sanitized input. Deliberately NOT ModelField.IsSystemField: that
// answers "does the server own this field's meaning" for a client rendering a form, whereas this
// answers "must this value be stripped from a write". A primary or versioning key is a system
// field by the first sense and still writable in the second, so the two must not be merged.
func (this *ModelSchema) isAutoGeneratedNonKey(field *ModelField) bool {
	return field.IsAutoGenerated() && !this.IsPrimaryKey(field.name) && !this.IsVersioningKey(field.name)
}

// isNoUpdate indicates a field can be written only on creation, but
...
func (this *ModelSchema) Validate(input DynamicFields, forEdit ...bool) (DynamicFields, ft.ClientErrors) {
	...
	for _, name := range this.fieldsOrder {
		...
		} else if this.isAutoGeneratedNonKey(field) || this.isNoUpdate(field, isForEdit) {
			continue
		}
...
func (this *ModelSchema) InjectServiceFields(ctx context.Context, result DynamicFields, forEdit bool) {
	for _, field := range this.fields {
		...
		if field.IsVirtual() || !field.IsServiceInjected() || field.injectFn == nil {
			continue
		}
...
func (this *ModelSchema) ToSimplized() any {
	...
	simplizedFields := make(map[string]any, len(this.Fields()))
	for name, field := range this.Fields() {
		simplizedFields[name] = field.ToSimplized()
	}
...
func (this ModelRelation) ToSimplized() any {
	return struct {
		...
		DestSchemaName: this.DestSchemaName,
...
type ModelField struct {
	...
	isPrimaryKey          bool
	isTenantKey           bool
	// Indicates the field is the local side of an edge's key map, or a junction-table FK column.
	// Set by SchemaRegistry.FinalizeRelations, which is the first point at which the owning
	// schema of every foreign key is known.
	isForeignKey bool
	isUnique     bool
	...
	// itself imports this package); computed.DefOf is the typed accessor.
	computedExpr any
	// Whether the value is computed at read time and therefore occupies no column. Stored
	// negated so the zero value means "persisted", which is right for an ordinary field and for
	// any ModelField built as a literal. Only the read-time form is implemented; Build() rejects
	// a stored computed field until a future phase.
	isNotPersisted bool
	rules          []*FieldRule
	defaultValue   *value
	defaultFn      func() any
...
// IsEdgeModel is true for FieldDataTypeModel: a model-typed field standing for a relation,
// hydrated from the peer schema rather than read from a column of its own.
...
// IsComputed is true when the field's value is derived rather than supplied by the client:
// either a declared computation, or an edge hydrated from the peer schema. It says nothing
// about whether the value occupies a column — that is IsPersisted.
func (this *ModelField) IsComputed() bool {
	if this == nil {
		return false
	}
	return this.computedExpr != nil || this.IsEdgeModel()
}

// IsPersisted is true when the field's value occupies a database column. True for every
// ordinary field; false for an edge and for a computed field evaluated at read time.
func (this *ModelField) IsPersisted() bool {
	if this == nil || this.IsEdgeModel() {
		return false
	}
	return !this.isNotPersisted
}

// IsVirtual is true when the field has no database column, for either reason: a read-time
// computed scalar or an edge. Use it wherever the question is "can this be written, filtered
// or ordered"; use IsEdgeModel when the answer differs between an edge and a virtual scalar.
...
// IsForeignKey is true when the field is the local side of an edge's key map, or a
// junction-table FK column.
func (this *ModelField) IsForeignKey() bool {
	return this != nil && this.isForeignKey
}

// IsSystemField is true for a field the server owns rather than the client: the keys that
// identify, version or scope a record, plus the foreign keys that wire it to its peers.
// Deliberately says nothing about whether the field has a column — that is IsVirtual.
func (this *ModelField) IsSystemField() bool {
	if this == nil {
		return false
	}
	return this.isPrimaryKey || this.isVersioningKey || this.isTenantKey || this.isForeignKey
}
...
func (this ModelField) ToSimplized() any {
	return struct {
		...
		IsEdgeModel         bool           `json:"is_edge_model"`
		...
		IsSystemField:       this.IsSystemField(),
		IsEdgeModel:         this.IsEdgeModel(),
...
func (this *ModelField) Clone() *ModelField {
	cloned := &ModelField{
		...
		isAutoGenerated:     this.isAutoGenerated,
		isPrimaryKey:        this.isPrimaryKey,
		isVersioningKey:     this.isVersioningKey,
		isTenantKey:         this.isTenantKey,
		isForeignKey:        this.isForeignKey,
		isUnique:            this.isUnique,
		computedExpr:        this.computedExpr,
		isNotPersisted:      this.isNotPersisted,
