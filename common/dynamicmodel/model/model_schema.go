...
func (this *ModelSchema) isReadOnlyOrSystemField(field *ModelField) bool {
	isSystem := field.IsAutoGenerated() && !this.IsPrimaryKey(field.name) && !this.IsVersioningKey(field.name)
	isReadOnly := field.IsNoUpdate()
	return isSystem || isReadOnly
}
...
func (this *ModelSchema) Validate(input DynamicFields, forEdit ...bool) (DynamicFields, ft.ClientErrors) {
	...
	for _, name := range this.fieldsOrder {
		...
		} else if this.isReadOnlyOrSystemField(field) {
			continue
		}
