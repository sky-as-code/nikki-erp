...
// AddConversion mothod allows registering a custom `Converter` into the global `converterMap`
func AddConversion[TIn any, TOut any](converter Converter) {
	var in TIn
	var out TOut
	AddConversionByType(reflect.TypeOf(in), reflect.TypeOf(out), converter)
}
...
func doCopy(dv, sv reflect.Value) []error {
	...
	for _, f := range fields {
		...
		srcTag := newTag(f.Tag.Get(TagName))

		if srcTag.isOmitField() {
			continue
		}

		// check type is in NoTraverseTypeList or has 'notraverse' tag option
		noTraverse := (isNoTraverseType(sfv) || srcTag.isNoTraverse())
		...
		// 	continue
		// }
		isSrcNil := isPtr(sfv) && sfv.IsNil()
		if isSrcNil && !srcTag.isOmitEmpty() {
			dfv.Set(zeroOf(dfv))
			continue
		}
