CREATE OR REPLACE FUNCTION iam_calc_user_perm(p_user_id varchar DEFAULT NULL)
RETURNS TABLE (
	-- The order must exactly match CREATE TABLE "iam_user_permissions"
	user_id                   varchar,
	ent_id                    varchar,
...
WITH role_entitlements AS (
	...
	FROM iam_role_user_assignments ra
		...
		LEFT JOIN iam_org_user_rel our ON our.user_id = ra.receiver_user_id
	...
	FROM iam_group_user_rel gur
		JOIN iam_role_group_assignments ra ON ra.receiver_group_id = gur.group_id AND (ra.expires_at IS NULL OR ra.expires_at > NOW())
		JOIN iam_roles r ON r.id = ra.role_id AND r.is_archived = FALSE
		JOIN iam_entitlements e ON e.role_id = r.id AND e.is_archived = FALSE
		LEFT JOIN iam_actions a ON a.id = e.action_id AND a.resource_id = e.resource_id
		LEFT JOIN iam_resources res ON res.id = a.resource_id
...
BEGIN
	...
	INSERT INTO iam_user_permissions
		SELECT * FROM iam_calc_user_perm(p_user_id)
...
AS $$
BEGIN
	TRUNCATE TABLE iam_user_permissions;

	INSERT INTO iam_user_permissions
		SELECT * FROM iam_calc_user_perm(NULL)
		ON CONFLICT DO NOTHING;
END $$;
