...
CREATE TABLE "ident_users" ("id" character varying NOT NULL, "avatar_url" character varying NULL, "created_at" timestamptz NOT NULL, "display_name" character varying NOT NULL, "email" character varying NOT NULL, "etag" character varying NOT NULL, "failed_login_attempts" bigint NOT NULL DEFAULT 0, "is_owner" boolean NULL, "last_login_at" timestamptz NULL, "locked_until" timestamptz NULL, "must_change_password" boolean NOT NULL DEFAULT true, "password_hash" character varying NOT NULL, "password_changed_at" timestamptz NOT NULL, "updated_at" timestamptz NULL, "hierarchy_id" character varying NULL, "status_id" character varying NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "ident_users_core_enums_user_status" FOREIGN KEY ("status_id") REFERENCES "core_enums" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "ident_users_ident_hierarchy_levels_hierarchy" FOREIGN KEY ("hierarchy_id") REFERENCES "ident_hierarchy_levels" ("id") ON UPDATE NO ACTION ON DELETE SET NULL);
