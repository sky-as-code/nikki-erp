...
BEGIN
	...
	IF EXISTS (
		...
		INSERT INTO "ident_users" ("id", "created_at", "display_name", "email", "etag", "is_owner", "is_archived", "status") VALUES
		('01JWNNJGS70Y07MBEV3AQ0M526', NOW(), 'System', 'system@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JWNMZ36QHC7CQQ748H9NQ6J6', NOW(), 'Admin Owner', 'owner@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, TRUE, FALSE, 'active'),
		('01JWNXT3EY7FG47VDJTEPTDC98', NOW(), 'Nguyễn Văn An', 'nguyen.van.an@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JWNXXTF8958VVYAV33MVVMDN', NOW(), 'Trần Thị Bình', 'tran.thi.binh@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JZQFDH0N51Q3BFQFMFFGSCSV', NOW(), 'Lê Văn Cường', 'le.van.cuong@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JZQFF9QEXH71P2CG9Y9MY8MM', NOW(), 'Phạm Thị Dung', 'pham.thi.dung@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'locked'),
		('01JZQFFDKY8T4JB8R6NSY1331J', NOW(), 'Hoàng Văn Em', 'hoang.van.em@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JZQFGVKZCTV7S310W0BDMWCS', NOW(), 'Đặng Thị Phương', 'dang.thi.phuong@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active'),
		('01JZQFY6EXRG0959Z95Y2EM3AM', NOW(), 'Võ Văn Giang', 'vo.van.giang@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'archived'),
		('01JZQFZFK6GM2D5X6MYHWH6FND', NOW(), 'Bùi Thị Hoa', 'bui.thi.hoa@nikki.com', (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint, NULL, FALSE, 'active');
	END IF;
