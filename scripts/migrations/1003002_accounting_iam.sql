...
BEGIN
	...
	IF EXISTS (
		...
		('01M4ACCT000000000000000001', 'Tax Jurisdiction', 'accounting_tax_jurisdiction', 'A territory in which a taxing authority levies tax', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000002', 'Tax Group', 'accounting_tax_group', 'A grouping of taxes for display and reporting; never a calculation formula', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000003', 'Tax Rounding Policy', 'accounting_tax_rounding_policy', 'How a computed tax amount is rounded, and whether per line or per document', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000004', 'Tax Product Classification', 'accounting_tax_product_classification', 'What a product is for tax purposes, which rules test rather than the product itself', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000005', 'Tax', 'accounting_tax', 'The stable business identity of a tax, as quoted on an invoice and in law', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000006', 'Tax Definition Version', 'accounting_tax_definition_version', 'Everything about a tax that affects determination or calculation, for one effective period', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000007', 'Tax Rate Version', 'accounting_tax_rate_version', 'The rate or fixed amount of a tax over one effective period', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000008', 'Tax Component', 'accounting_tax_component', 'One child tax inside a group tax, versioned with the definition that composes it', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000009', 'Tax Mapping', 'accounting_tax_mapping', 'A context-specific substitution of one tax for another', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000010', 'Tax Mapping Line', 'accounting_tax_mapping_line', 'One source-tax to target-tax substitution within a mapping', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000011', 'Tax Rule', 'accounting_tax_rule', 'A rule deciding which taxes apply to a transaction context', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000012', 'Tax Rule Condition', 'accounting_tax_rule_condition', 'One typed predicate on the tax context', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text),
		('01M4ACCT000000000000000013', 'Tax Rule Result', 'accounting_tax_rule_result', 'What a matching rule does to the candidate tax set', 'nikkierp', 'tenant', 'org', NOW(), (EXTRACT(EPOCH FROM clock_timestamp()) * 1e9)::bigint::text)
