...
func (tx *Tx) init() {
	tx.AuthNEnum = NewAuthNEnumClient(tx.config)
	tx.LoginAttempt = NewLoginAttemptClient(tx.config)
	tx.MethodSetting = NewMethodSettingClient(tx.config)
	tx.PasswordStore = NewPasswordStoreClient(tx.config)
}
...
// applies a query, for example: AuthNEnum.QueryXXX(), the query will be executed
