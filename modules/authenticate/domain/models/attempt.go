...
const (
	LoginAttemptSchemaName = "authenticate_login_attempt"
...
func LoginAttemptSchemaBuilder() *dmodel.ModelSchemaBuilder {
	return dmodel.DefineModel(LoginAttemptSchemaName).
		Label(model.NewLangJsonRefSf("%s.label", LoginAttemptSchemaName)).
		...
		Field(
			dmodel.DefineField().Name(AttemptFieldMethods).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldMethods)).
		...
		Field(
			dmodel.DefineField().Name(AttemptFieldCurrentMethod).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldCurrentMethod)).
		...
		Field(
			dmodel.DefineField().Name(AttemptFieldDeviceIp).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldDeviceIp)).
				DataType(dmodel.FieldDataTypeString(0, 45)),
		).
		Field(
			DefinePrincipalDeviceNameField().
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldDeviceName)),
		).
		Field(
			dmodel.DefineField().Name(AttemptFieldDeviceLocation).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldDeviceLocation)).
				DataType(dmodel.FieldDataTypeString(0, model.MODEL_RULE_SHORT_NAME_LENGTH)),
		).
		Field(
			dmodel.DefineField().Name(AttemptFieldExpiresAt).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldExpiresAt)).
		...
		Field(
			DefinePrincipalTypeField(AttemptFieldPrincipalType).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldPrincipalType)).
				Default(PrincipalTypeNikkiUser).
				RequiredForCreate(),
		).
		Field(
			dmodel.DefineField().Name(AttemptFieldStatus).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldStatus)).
		...
		Field(
			DefinePrincipalUsernameField(AttemptFieldUsername).
				Label(model.NewLangJsonRefSf("fields.%s", AttemptFieldUsername)).
				RequiredForCreate(),
		)
}
