...
func NewActionServiceImpl(
	...
	resourceSvc itRes.ResourceService,
...
func (this *ActionServiceImpl) CreateAction(
	...
	return corecrud.Create(ctx, corecrud.CreateParam[domain.Action, *domain.Action]{
		...
		BeforeValidation: func(ctx corectx.Context, inputModel *domain.Action, vErrs *ft.ClientErrors) (*domain.Action, error) {
			err := this.checkResourceExists(ctx, inputModel.GetResourceId(), vErrs)
			if err != nil {
				return nil, err
			}
...
func (this *ActionServiceImpl) existsQueryKeys(query itAct.ActionExistsQuery) []domain.Action {
...
func (this *ActionServiceImpl) existsResultData(repo dyn.RepoExistsResult) dyn.ExistsResultData {
...
func (this *ActionServiceImpl) GetAction(
...
) (_ *itAct.GetActionResult, err error) {
	defer func() {
	...
	sanitized, cErrs := query.GetSchema().ValidateStruct(query)
	if cErrs.Count() > 0 {
		return &itAct.GetActionResult{ClientErrors: cErrs}, nil
	}
	...
	if resSearch.ClientErrors.Count() > 0 {
		return &itAct.GetActionResult{ClientErrors: resSearch.ClientErrors}, nil
	}

	var result itAct.GetActionResult
...
func (this *ActionServiceImpl) checkResourceExists(ctx corectx.Context, resourceId *model.Id, cErrs *ft.ClientErrors) error {
	resrcExists, err := this.resourceSvc.ResourceExists(ctx, itRes.ResourceExistsQuery{
		Ids: []model.Id{*resourceId},
	})
	if err != nil {
	...
	if !resrcExists.Data.Exists(*resourceId) {
