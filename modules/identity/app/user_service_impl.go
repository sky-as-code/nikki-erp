...
func (this *UserServiceImpl) UpdateUser(ctx context.Context, cmd it.UpdateUserCommand) (result *it.UpdateUserResult, err error) {
	...
	vErrs, err := flow.
		...
		Step(func(vErrs *ft.ValidationErrors) error {
			dbUser, err = this.assertUserExists(ctx, *user.Id, nil, vErrs)
			return err
		}).
		Step(func(vErrs *ft.ValidationErrors) error {
...
func (this *UserServiceImpl) assertUserExists(ctx context.Context, id model.Id, status *domain.UserStatus, vErrs *ft.ValidationErrors) (dbUser *domain.User, err error) {
...
func (this *UserServiceImpl) GetUserById(ctx context.Context, query it.GetUserByIdQuery) (result *it.GetUserByIdResult, err error) {
	...
	vErrs, err := flow.
		...
		Step(func(vErrs *ft.ValidationErrors) error {
			dbUser, err = this.assertUserExists(ctx, query.Id, query.Status, vErrs)
			return err
		}).
		End()
	ft.PanicOnErr(err)
