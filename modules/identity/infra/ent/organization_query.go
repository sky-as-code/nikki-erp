...
func (oq *OrganizationQuery) QueryUserOrgs() *UserOrgQuery {
	query := (&UserOrgClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithUserOrgs(opts ...func(*UserOrgQuery)) *OrganizationQuery {
	query := (&UserOrgClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withUserOrgs = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withUserOrgs; query != nil {
		if err := oq.loadUserOrgs(ctx, query, nodes,
			func(n *Organization) { n.Edges.UserOrgs = []*UserOrg{} },
			func(n *Organization, e *UserOrg) { n.Edges.UserOrgs = append(n.Edges.UserOrgs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadUserOrgs(ctx context.Context, query *UserOrgQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *UserOrg)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userorg.FieldOrgID)
	}
	query.Where(predicate.UserOrg(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.UserOrgsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "org_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
