...
func (uq *UserQuery) QueryHierarchy() *HierarchyLevelQuery {
	query := (&HierarchyLevelClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryOrgs() *OrganizationQuery {
...
// QueryUserOrgs chains the current query on the "user_orgs" edge.
func (uq *UserQuery) QueryUserOrgs() *UserOrgQuery {
	query := (&UserOrgClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithHierarchy(opts ...func(*HierarchyLevelQuery)) *UserQuery {
	query := (&HierarchyLevelClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withHierarchy = query
	return uq
}
...
func (uq *UserQuery) WithOrgs(opts ...func(*OrganizationQuery)) *UserQuery {
	query := (&OrganizationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithUserOrgs tells the query-builder to eager-load the nodes that are connected to
// the "user_orgs" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserOrgs(opts ...func(*UserOrgQuery)) *UserQuery {
	query := (&UserOrgClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserOrgs = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUserOrgs; query != nil {
		if err := uq.loadUserOrgs(ctx, query, nodes,
			func(n *User) { n.Edges.UserOrgs = []*UserOrg{} },
			func(n *User, e *UserOrg) { n.Edges.UserOrgs = append(n.Edges.UserOrgs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadOrgs(ctx context.Context, query *OrganizationQuery, nodes []*User, init func(*User), assign func(*User, *Organization)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*User)
	nids := make(map[string]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(organization.FieldID), joinT.C(user.OrgsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.OrgsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.OrgsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (uq *UserQuery) loadUserOrgs(ctx context.Context, query *UserOrgQuery, nodes []*User, init func(*User), assign func(*User, *UserOrg)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userorg.FieldUserID)
	}
	query.Where(predicate.UserOrg(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserOrgsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
