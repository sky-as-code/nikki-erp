...
func (aq *ActionQuery) QueryEntitlements() *EntitlementQuery {
...
func (aq *ActionQuery) QueryResource() *ResourceQuery {
...
func (aq *ActionQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (aq *ActionQuery) FirstIDX(ctx context.Context) string {
...
func (aq *ActionQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (aq *ActionQuery) OnlyIDX(ctx context.Context) string {
...
func (aq *ActionQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (aq *ActionQuery) IDsX(ctx context.Context) []string {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *ActionQuery) WithEntitlements(opts ...func(*EntitlementQuery)) *ActionQuery {
	query := (&EntitlementClient{config: aq.config}).Query()
...
func (aq *ActionQuery) WithResource(opts ...func(*ResourceQuery)) *ActionQuery {
	query := (&ResourceClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ActionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Action, error) {
	...
	if query := aq.withEntitlements; query != nil {
		if err := aq.loadEntitlements(ctx, query, nodes,
			func(n *Action) { n.Edges.Entitlements = []*Entitlement{} },
			func(n *Action, e *Entitlement) { n.Edges.Entitlements = append(n.Edges.Entitlements, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withResource; query != nil {
		if err := aq.loadResource(ctx, query, nodes, nil,
			func(n *Action, e *Resource) { n.Edges.Resource = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ActionQuery) loadEntitlements(ctx context.Context, query *EntitlementQuery, nodes []*Action, init func(*Action), assign func(*Action, *Entitlement)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Action)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entitlement.FieldActionID)
	}
	query.Where(predicate.Entitlement(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(action.EntitlementsColumn), fks...))
	}))
...
func (aq *ActionQuery) loadResource(ctx context.Context, query *ResourceQuery, nodes []*Action, init func(*Action), assign func(*Action, *Resource)) error {
...
func (aq *ActionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(action.Table, action.Columns, sqlgraph.NewFieldSpec(action.FieldID, field.TypeString))
	_spec.From = aq.sql
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withResource != nil {
			_spec.Node.AddColumnOnce(action.FieldResourceID)
		}
	}
