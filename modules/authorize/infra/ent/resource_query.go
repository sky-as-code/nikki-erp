...
func (rq *ResourceQuery) QueryActions() *ActionQuery {
...
func (rq *ResourceQuery) QueryEntitlements() *EntitlementQuery {
	query := (&EntitlementClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withEntitlements: rq.withEntitlements.Clone(),
...
func (rq *ResourceQuery) WithActions(opts ...func(*ActionQuery)) *ResourceQuery {
	query := (&ActionClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) WithEntitlements(opts ...func(*EntitlementQuery)) *ResourceQuery {
	query := (&EntitlementClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withEntitlements = query
	return rq
}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := rq.withActions; query != nil {
		if err := rq.loadActions(ctx, query, nodes,
			func(n *Resource) { n.Edges.Actions = []*Action{} },
			func(n *Resource, e *Action) { n.Edges.Actions = append(n.Edges.Actions, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withEntitlements; query != nil {
		if err := rq.loadEntitlements(ctx, query, nodes,
			func(n *Resource) { n.Edges.Entitlements = []*Entitlement{} },
			func(n *Resource, e *Entitlement) { n.Edges.Entitlements = append(n.Edges.Entitlements, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResourceQuery) loadActions(ctx context.Context, query *ActionQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Action)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(action.FieldResourceID)
	}
	query.Where(predicate.Action(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.ActionsColumn), fks...))
	}))
...
func (rq *ResourceQuery) loadEntitlements(ctx context.Context, query *EntitlementQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Entitlement)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Resource)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entitlement.FieldResourceID)
	}
	query.Where(predicate.Entitlement(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.EntitlementsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
