...
// 			return this.actionRepo.DeleteHard(ctx, it.DeleteParam{Id: *model.GetId()})
// 		},
...
// 		s := defense.SanitizePlainText(*action.GetDescription(), true)
// 		action.SetDescription(&s)
// 	}

// 	if action.GetName() != nil {
// 		s := defense.SanitizePlainText(*action.GetName(), true)
// 		action.SetName(&s)
// 	}
// }
...
// 	err = this.assertResourceExists(ctx, *action.GetResourceId(), vErrs)
// 	fault.PanicOnErr(err)
...
// 	dbAction, err := this.actionRepo.FindByName(ctx, it.FindByNameParam{
// 		Name:       *action.GetName(),
...
// 	dbAction, err = this.actionRepo.FindById(ctx, it.FindByIdParam{Id: *action.GetId()})
// 	fault.PanicOnErr(err)
...
// func (this *ActionServiceImpl) assertConstraintViolated(ctx crud.Context, action *domain.Action, vErrs *fault.ValidationErrors) error {
// 	names, err := this.actionRepo.ListEntitlementNamesForActionId(ctx, *action.GetId())
// 	fault.PanicOnErr(err)
// 	for _, n := range names {
...
// 	return this.assertConstraintViolated(ctx, action, vErrs)
// }
