...
// 		if grantRequest.Status != nil && *grantRequest.Status == domain.GrantRequestStatusPending {
// 			cancelStatus := domain.GrantRequestStatusCancelled
...
// 			if dbGrantRequest != nil && *dbGrantRequest.Status != domain.GrantRequestStatusPending {
...
// 	grantRequest.Etag = model.NewEtag()
// 	status := domain.GrantRequestStatusCancelled
...
// func (this *GrantRequestServiceImpl) GetGrantRequestById(ctx crud.Context, query itGrantRequest.GetGrantRequestByIdQuery) (result *itGrantRequest.GetGrantRequestByIdResult, err error) {
// 	defer func() {
// 		if e := fault.RecoverPanicFailedTo(recover(), "get grant request by Id"); e != nil {
// 			err = e
// 		}
// 	}()

// 	var vErrs fault.ValidationErrors
// 	vErrs.Merge(query.Validate())
// 	if vErrs.Count() > 0 {
// 		return &itGrantRequest.GetGrantRequestByIdResult{ClientError: vErrs.ToClientError()}, nil
// 	}
...
// 	responses, err := this.grantResponseRepo.FindByRequestId(ctx, *dbGrantRequest.Id)
// 	fault.PanicOnErr(err)
// 	for i := range responses {
// 		responses[i].ResponderName, err = this.getUserDisplayName(ctx, *responses[i].ResponderId, "user", nil)
// 		fault.PanicOnErr(err)
// 	}

// 	return &itGrantRequest.GetGrantRequestByIdResult{
// 		Data: &itGrantRequest.GrantRequestByIdData{
// 			GrantRequest:   dbGrantRequest,
// 			GrantResponses: responses,
// 		},
// 		HasData: true,
// 	}, nil
// }
...
// 	targetOrgId = role.GetOrgId()
// 	this.validateTarget(role.GetIsRequestable(), role.GetIsRequiredAttachment(), role.GetIsRequiredComment(), grantRequest, vErrs)
...
// 	case domain.GrantReqReceiverTypeUser:
...
// 		}
// 		return
// 	case domain.GrantRequestReceiverTypeGroup:
...
// 	pendingRequests, err := this.grantRequestRepo.FindPendingByReceiverAndTarget(ctx, cmd.ReceiverId, cmd.TargetRef)
// 	fault.PanicOnErr(err)

// 	if len(pendingRequests) > 0 {
// 		vErrs.AppendAlreadyExists("receiverId", "receiver already has a pending request for this rolealready exists")
// 	}
...
// 	status := domain.GrantRequestStatusPending
// 	grantRequest.Status = &status
...
// 	if *grantRequest.ReceiverType == domain.GrantRequestReceiverTypeGroup {
...
// 	role, err := this.roleRepo.FindById(ctx, itRole.FindByIdParam{Id: model.Id(targetId)})
// 	fault.PanicOnErr(err)
...
// 		refStr = &s
// 	}
// 	return refStr, &ownerType, nil
// }

// func (this *GrantRequestServiceImpl) findOwnerUserIds(ctx crud.Context, targetId string, vErrs *fault.ValidationErrors) ([]string, error) {
// 	ownerId, ownerType, err := this.findOwnerInfo(ctx, targetId, vErrs)
// 	fault.PanicOnErr(err)
...
// 		rejectedStatus := domain.GrantRequestStatusRejected
...
// 	approvedStatus := domain.GrantRequestStatusApproved
...
// 	}
// 	grantResponse.ModelBase.SetDefaults()
...
// 	if *dbGrantRequest.Status != domain.GrantRequestStatusApproved {
// 		return deletion, err
// 	}
...
// 	if *dbGrantRequest.ReceiverType == domain.GrantRequestReceiverTypeGroup {
