...
// The ModuleOrgRelFunc type is an adapter to allow the use of ordinary
// function as ModuleOrgRel mutator.
type ModuleOrgRelFunc func(context.Context, *ent.ModuleOrgRelMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ModuleOrgRelFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ModuleOrgRelMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ModuleOrgRelMutation", m)
}
