// Code generated by ent, DO NOT EDIT.

package migrate

import (
	"entgo.io/ent/dialect/entsql"
	"entgo.io/ent/dialect/sql/schema"
	"entgo.io/ent/schema/field"
)

var (
	// DriFilesColumns holds the columns for the "dri_files" table.
	DriFilesColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString},
		{Name: "etag", Type: field.TypeString},
		{Name: "created_at", Type: field.TypeTime},
		{Name: "updated_at", Type: field.TypeTime},
		{Name: "deleted_at", Type: field.TypeTime},
		{Name: "owner_ref", Type: field.TypeString},
		{Name: "name", Type: field.TypeString},
		{Name: "mime", Type: field.TypeString},
		{Name: "is_folder", Type: field.TypeBool, Default: false},
		{Name: "size", Type: field.TypeInt64},
		{Name: "storage_path", Type: field.TypeString},
		{Name: "storage_key", Type: field.TypeString},
		{Name: "storage", Type: field.TypeString},
		{Name: "visibility", Type: field.TypeString, Default: "owner"},
		{Name: "status", Type: field.TypeString, Default: "active"},
		{Name: "parent_file_ref", Type: field.TypeString, Nullable: true},
	}
	// DriFilesTable holds the schema information for the "dri_files" table.
	DriFilesTable = &schema.Table{
		Name:       "dri_files",
		Columns:    DriFilesColumns,
		PrimaryKey: []*schema.Column{DriFilesColumns[0]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "dri_files_dri_files_children_file",
				Columns:    []*schema.Column{DriFilesColumns[15]},
				RefColumns: []*schema.Column{DriFilesColumns[0]},
				OnDelete:   schema.SetNull,
			},
		},
		Indexes: []*schema.Index{
			{
				Name:    "drivefile_owner_ref_parent_file_ref_is_folder_name",
				Unique:  true,
				Columns: []*schema.Column{DriFilesColumns[5], DriFilesColumns[15], DriFilesColumns[8], DriFilesColumns[6]},
				Annotation: &entsql.IndexAnnotation{
					Where: "parent_file_ref is NOT NULL",
				},
			},
			{
				Name:    "drivefile_owner_ref_is_folder_name",
				Unique:  true,
				Columns: []*schema.Column{DriFilesColumns[5], DriFilesColumns[8], DriFilesColumns[6]},
			},
			{
				Name:    "drivefile_parent_file_ref",
				Unique:  false,
				Columns: []*schema.Column{DriFilesColumns[15]},
			},
			{
				Name:    "drivefile_status",
				Unique:  false,
				Columns: []*schema.Column{DriFilesColumns[14]},
			},
		},
	}
	// DriFileAncestorsColumns holds the columns for the "dri_file_ancestors" table.
	DriFileAncestorsColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString},
		{Name: "ancestor_ref", Type: field.TypeString},
		{Name: "depth", Type: field.TypeInt, Default: 0},
		{Name: "file_ref", Type: field.TypeString},
	}
	// DriFileAncestorsTable holds the schema information for the "dri_file_ancestors" table.
	DriFileAncestorsTable = &schema.Table{
		Name:       "dri_file_ancestors",
		Columns:    DriFileAncestorsColumns,
		PrimaryKey: []*schema.Column{DriFileAncestorsColumns[0]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "dri_file_ancestors_dri_files_drive_file_ancestors",
				Columns:    []*schema.Column{DriFileAncestorsColumns[3]},
				RefColumns: []*schema.Column{DriFilesColumns[0]},
				OnDelete:   schema.NoAction,
			},
		},
		Indexes: []*schema.Index{
			{
				Name:    "drivefileancestor_file_ref_ancestor_ref",
				Unique:  true,
				Columns: []*schema.Column{DriFileAncestorsColumns[3], DriFileAncestorsColumns[1]},
			},
			{
				Name:    "drivefileancestor_ancestor_ref",
				Unique:  false,
				Columns: []*schema.Column{DriFileAncestorsColumns[1]},
			},
		},
	}
	// DriFileSharesColumns holds the columns for the "dri_file_shares" table.
	DriFileSharesColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString},
		{Name: "etag", Type: field.TypeString},
		{Name: "created_at", Type: field.TypeTime},
		{Name: "updated_at", Type: field.TypeTime},
		{Name: "user_ref", Type: field.TypeString},
		{Name: "permission", Type: field.TypeString, Default: "view"},
		{Name: "file_ref", Type: field.TypeString},
	}
	// DriFileSharesTable holds the schema information for the "dri_file_shares" table.
	DriFileSharesTable = &schema.Table{
		Name:       "dri_file_shares",
		Columns:    DriFileSharesColumns,
		PrimaryKey: []*schema.Column{DriFileSharesColumns[0]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "dri_file_shares_dri_files_drive_file_shares",
				Columns:    []*schema.Column{DriFileSharesColumns[6]},
				RefColumns: []*schema.Column{DriFilesColumns[0]},
				OnDelete:   schema.NoAction,
			},
		},
		Indexes: []*schema.Index{
			{
				Name:    "drivefileshare_file_ref_user_ref",
				Unique:  true,
				Columns: []*schema.Column{DriFileSharesColumns[6], DriFileSharesColumns[4]},
			},
		},
	}
	// DriFileStarsColumns holds the columns for the "dri_file_stars" table.
	DriFileStarsColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString},
		{Name: "etag", Type: field.TypeString},
		{Name: "created_at", Type: field.TypeTime},
		{Name: "updated_at", Type: field.TypeTime},
		{Name: "user_ref", Type: field.TypeString},
		{Name: "file_ref", Type: field.TypeString},
	}
	// DriFileStarsTable holds the schema information for the "dri_file_stars" table.
	DriFileStarsTable = &schema.Table{
		Name:       "dri_file_stars",
		Columns:    DriFileStarsColumns,
		PrimaryKey: []*schema.Column{DriFileStarsColumns[0]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "dri_file_stars_dri_files_drive_file_stars",
				Columns:    []*schema.Column{DriFileStarsColumns[5]},
				RefColumns: []*schema.Column{DriFilesColumns[0]},
				OnDelete:   schema.NoAction,
			},
		},
		Indexes: []*schema.Index{
			{
				Name:    "drivefilestar_file_ref_user_ref",
				Unique:  true,
				Columns: []*schema.Column{DriFileStarsColumns[5], DriFileStarsColumns[4]},
			},
		},
	}
	// Tables holds all the tables in the schema.
	Tables = []*schema.Table{
		DriFilesTable,
		DriFileAncestorsTable,
		DriFileSharesTable,
		DriFileStarsTable,
	}
)

func init() {
	DriFilesTable.ForeignKeys[0].RefTable = DriFilesTable
	DriFilesTable.Annotation = &entsql.Annotation{
		Table: "dri_files",
	}
	DriFileAncestorsTable.ForeignKeys[0].RefTable = DriFilesTable
	DriFileAncestorsTable.Annotation = &entsql.Annotation{
		Table: "dri_file_ancestors",
	}
	DriFileSharesTable.ForeignKeys[0].RefTable = DriFilesTable
	DriFileSharesTable.Annotation = &entsql.Annotation{
		Table: "dri_file_shares",
	}
	DriFileStarsTable.ForeignKeys[0].RefTable = DriFilesTable
	DriFileStarsTable.Annotation = &entsql.Annotation{
		Table: "dri_file_stars",
	}
}
