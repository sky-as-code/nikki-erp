...
type AttributeQuery struct {
	...
	withAttributeValues *AttributeValueQuery
...
func (aq *AttributeQuery) QueryAttributeGroup() *AttributeGroupQuery {
	query := (&AttributeGroupClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) QueryProduct() *ProductQuery {
...
func (aq *AttributeQuery) QueryAttributeValues() *AttributeValueQuery {
...
func (aq *AttributeQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (aq *AttributeQuery) FirstIDX(ctx context.Context) string {
...
func (aq *AttributeQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (aq *AttributeQuery) OnlyIDX(ctx context.Context) string {
...
func (aq *AttributeQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (aq *AttributeQuery) IDsX(ctx context.Context) []string {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AttributeQuery) Clone() *AttributeQuery {
	...
	return &AttributeQuery{
		...
		withAttributeGroup:  aq.withAttributeGroup.Clone(),
		withProduct:         aq.withProduct.Clone(),
		withAttributeValues: aq.withAttributeValues.Clone(),
...
func (aq *AttributeQuery) WithAttributeGroup(opts ...func(*AttributeGroupQuery)) *AttributeQuery {
	query := (&AttributeGroupClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withAttributeGroup = query
	return aq
}

// WithProduct tells the query-builder to eager-load the nodes that are connected to
// the "product" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AttributeQuery) WithProduct(opts ...func(*ProductQuery)) *AttributeQuery {
	query := (&ProductClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AttributeQuery) WithAttributeValues(opts ...func(*AttributeValueQuery)) *AttributeQuery {
	query := (&AttributeValueClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withAttributeValues = query
	return aq
}
...
//		GroupBy(attribute.FieldCodeName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Attribute.Query().
//		Select(attribute.FieldCodeName).
//		Scan(ctx, &v)
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [3]bool{
			aq.withAttributeGroup != nil,
			aq.withProduct != nil,
			aq.withAttributeValues != nil,
		}
	)
	...
	if query := aq.withAttributeGroup; query != nil {
		if err := aq.loadAttributeGroup(ctx, query, nodes, nil,
			func(n *Attribute, e *AttributeGroup) { n.Edges.AttributeGroup = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withProduct; query != nil {
		if err := aq.loadProduct(ctx, query, nodes, nil,
			func(n *Attribute, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withAttributeValues; query != nil {
		if err := aq.loadAttributeValues(ctx, query, nodes,
			func(n *Attribute) { n.Edges.AttributeValues = []*AttributeValue{} },
			func(n *Attribute, e *AttributeValue) { n.Edges.AttributeValues = append(n.Edges.AttributeValues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadAttributeGroup(ctx context.Context, query *AttributeGroupQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *AttributeGroup)) error {
	...
	query.Where(attributegroup.IDIn(ids...))
...
func (aq *AttributeQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Product)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Attribute)
	for i := range nodes {
...
func (aq *AttributeQuery) loadAttributeValues(ctx context.Context, query *AttributeValueQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *AttributeValue)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributevalue.FieldAttributeID)
	}
	query.Where(predicate.AttributeValue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attribute.AttributeValuesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (aq *AttributeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(attribute.Table, attribute.Columns, sqlgraph.NewFieldSpec(attribute.FieldID, field.TypeString))
	_spec.From = aq.sql
