...
// The VariantAttributeRelFunc type is an adapter to allow the use of ordinary
// function as VariantAttributeRel mutator.
type VariantAttributeRelFunc func(context.Context, *ent.VariantAttributeRelMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VariantAttributeRelFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VariantAttributeRelMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VariantAttributeRelMutation", m)
}
