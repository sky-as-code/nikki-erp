...
const (
	// Every minute: sales and the kiosk fleet wait on these events, and an empty sweep
	// costs one indexed query.
	cronOutboxSweep = "* * * * *"

	jobNameOutboxSweep = "inventory-outbox-sweep"
...
// the sweep was also the delivery path, so the consumer of a reservation event waited for the next tick,
// up to a minute.
//
	...
	go func() {
		defer func() {
			...
			if recovered := recover(); recovered != nil {
				this.logError("inventory outbox: draining panicked", "", fmt.Errorf("%v", recovered))
			}
		}()

		if err := this.drain(drainCtx, drainPageSize); err != nil {
			this.logError("inventory outbox: draining failed", "", err)
		}
	}()
}
...
func (this *OutboxJobs) drain(ctx corectx.Context, limit int) error {
	...
	for _, row := range pending {
		...
		if err := this.publisher.Publish(publishCtx, event.IntegrationEvent); err != nil {
			this.logError("inventory outbox: publishing event failed", event.EventId, err)
			...
			if recordErr := services.RecordPublishFailure(ctx, row, err.Error()); recordErr != nil {
				this.logError("inventory outbox: recording a publish failure failed",
		...
		if err := services.MarkEventPublished(ctx, event.RowId); err != nil {
			...
			this.logError("inventory outbox: event published but not marked", event.EventId, err)
		}
	}
	return nil
}
