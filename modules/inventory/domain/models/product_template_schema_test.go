...
func TestProductTemplateSchemaParses(t *testing.T) {
	...
	// A template must be classifiable: type and category are mandatory, brand optional.
...
func TestProductVariantSchemaParses(t *testing.T) {
	...
	// Every variant belongs to exactly one template.
	assert.True(t, requireField(t, schema, ProductVariantFieldProductTemplateId).IsRequiredForCreate())
}

// Template-owned fields must not be duplicated onto the variant; a copy would let a variant drift
// from its template, the failure the Template/Variant split exists to prevent.
...
// The display name is computed from template name plus attribute values; storing it would let it
// drift out of step with the combination it describes.
...
// The database unique is unconditional; the engine additionally scopes it to non-archived rows so
// an archived variant keeps its combination for history while a replacement takes it over.
...
// status and is_archived are different concepts and both need their own field; collapsing them
// makes "discontinued but still listed" unrepresentable.
...
// Unarchiving a template must restore only what it cascaded to.
...
// Archiving and discontinuing each independently withdraw a variant from new transactions.
...
// A variant references the template-scoped value, not the global one, so that "every value is
// allowed by the template" is enforceable by the data model.
