		...
		Field(
			dmodel.DefineField().
				...
				DataType(dmodel.FieldDataTypeEnumString([]string{
					string(UserStatusDraft), string(UserStatusInvited),
					string(UserStatusActive), string(UserStatusSuspended),
				})).
				RequiredForCreate().
...
func (this User) GetOrgIds() []model.Id {
	...
	return array.Map(orgs, func(org Organization) model.Id {
