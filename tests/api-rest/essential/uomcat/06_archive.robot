...
Archive Succeeds
    ...
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'etag': $UOMCAT_ETAG, 'is_archived': True} }}
...
Unarchive Succeeds
    ${resp}=    POST On Session    api    ${UOMCAT_API}/${UOMCAT_ID}/archived
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'etag': $UOMCAT_ETAG, 'is_archived': False} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${UOMCAT_ETAG}
    IF    $etag is not None    Set Global Variable    ${UOMCAT_ETAG}    ${etag}
...
Archive With Not Found Id Fails
    ...
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'etag': $UOMCAT_ETAG, 'is_archived': True} }}    expected_status=any
