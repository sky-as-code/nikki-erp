...
Get With Columns Succeeds
    ${resp}=    GET On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    params=${{ {'org_id': $UOM_ORG_ID, 'fields': ['name', 'reference_uom_id']} }}
...
Get With Edge Columns Succeeds
    ...
    ${resp}=    GET On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    params=${{ {'org_id': $UOM_ORG_ID, 'fields': ['name', 'uoms.symbol']} }}
    Response Status Should Be    ${resp}    200

Get With Nonexist Column Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    params=${{ {'org_id': $UOM_ORG_ID, 'fields': ['name', 'bla_bla_field']} }}    expected_status=any
    Response Should Be Nonexist Fields Error    ${resp}    bla_bla_field

Get With Not Found Id Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${UOMCAT_API}/${NOT_FOUND_ID}
    ...    params=${{ {'org_id': $UOM_ORG_ID} }}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Get With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${UOMCAT_API}/not-existing-1234567890123
    ...    params=${{ {'org_id': $UOM_ORG_ID} }}    expected_status=any
