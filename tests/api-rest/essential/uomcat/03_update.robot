...
Update Succeeds
    ...
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'name': {'en-US': $name}, 'etag': $UOMCAT_ETAG} }}
...
Set Reference Uom Succeeds
    ...
    ${resp}=    PATCH On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'reference_uom_id': $REFERENCE_UOM_ID, 'etag': $UOMCAT_ETAG} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${UOMCAT_ETAG}
    IF    $etag is not None    Set Global Variable    ${UOMCAT_ETAG}    ${etag}
...
Update With Foreign Reference Uom Fails
    ...
    ${resp}=    PATCH On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'reference_uom_id': $FOREIGN_UOM_ID, 'etag': $UOMCAT_ETAG} }}
    ...    expected_status=any
...
Update With Unmatched Etag Fails
    ...
    ${resp}=    PATCH On Session    api    ${UOMCAT_API}/${UOMCAT_ID}
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'name': {'en-US': $name}, 'etag': '___________________'} }}    expected_status=any
...
Update With Invalid Id Format Fails
    ...
    ...    json=${{ {'org_id': $UOM_ORG_ID, 'etag': $UOMCAT_ETAG} }}    expected_status=any
