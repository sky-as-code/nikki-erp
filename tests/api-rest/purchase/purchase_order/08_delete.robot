...
Delete Succeeds Once The Order Is Cancelled
    ${id}    ${etag}=    Create Purchase Order
    POST On Session    api    ${PURCHASE_ORDER_API}/${id}/cancel    json=${{ {'org_id': $PURCHASE_ORG_ID, 'reason': 'not needed'} }}
    ${resp}=    DELETE On Session    api    ${PURCHASE_ORDER_API}/${id}
    ...    params=${{ {'org_id': $PURCHASE_ORG_ID} }}
...
Delete The Order Under Test
    ...
    POST On Session    api    ${PURCHASE_ORDER_API}/${PURCHASE_ORDER_ID}/cancel    json=${{ {'org_id': $PURCHASE_ORG_ID, 'reason': 'suite cleanup'} }}    expected_status=any
    ${resp}=    DELETE On Session    api    ${PURCHASE_ORDER_API}/${PURCHASE_ORDER_ID}
    ...    params=${{ {'org_id': $PURCHASE_ORG_ID} }}    expected_status=any
