...
Archive Succeeds
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $PARTY_ETAG, 'is_archived': True} }}
...
Unarchive Succeeds
    ${resp}=    POST On Session    api    ${PARTY_API}/${PARTY_ID}/archived
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $PARTY_ETAG, 'is_archived': False} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${PARTY_ETAG}
    IF    $etag is not None    Set Global Variable    ${PARTY_ETAG}    ${etag}
...
Archive With Not Found Id Fails
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $PARTY_ETAG, 'is_archived': True} }}    expected_status=any
