...
Update Succeeds
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'display_name': $name, 'etag': $PARTY_ETAG} }}
...
Update Website Succeeds
    ...
    ${resp}=    PATCH On Session    api    ${PARTY_API}/${PARTY_ID}
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'website': $website, 'etag': $PARTY_ETAG} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${PARTY_ETAG}
    IF    $etag is not None    Set Global Variable    ${PARTY_ETAG}    ${etag}
...
Update Type Succeeds
    ...
    ...    ordinary edit.
    ${resp}=    PATCH On Session    api    ${PARTY_API}/${PARTY_ID}
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'type': 'company', 'etag': $PARTY_ETAG} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${PARTY_ETAG}
    IF    $etag is not None    Set Global Variable    ${PARTY_ETAG}    ${etag}
    ${resp}=    GET On Session    api    ${PARTY_API}/${PARTY_ID}
    ...    params=${{ {'org_id': $CONTACTS_ORG_ID} }}
...
Update With Unmatched Etag Fails
    ...
    ${resp}=    PATCH On Session    api    ${PARTY_API}/${PARTY_ID}
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'display_name': $name, 'etag': '___________________'} }}    expected_status=any
...
Update With Invalid Id Format Fails
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $PARTY_ETAG} }}    expected_status=any
