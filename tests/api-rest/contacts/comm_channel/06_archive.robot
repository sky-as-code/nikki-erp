...
Archive Succeeds
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $COMM_CHANNEL_ETAG, 'is_archived': True} }}
...
Unarchive Succeeds
    ${resp}=    POST On Session    api    ${COMM_CHANNEL_API}/${COMM_CHANNEL_ID}/archived
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $COMM_CHANNEL_ETAG, 'is_archived': False} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${COMM_CHANNEL_ETAG}
    IF    $etag is not None    Set Global Variable    ${COMM_CHANNEL_ETAG}    ${etag}
...
Archive With Not Found Id Fails
    ...
    ...    json=${{ {'org_id': $CONTACTS_ORG_ID, 'etag': $COMM_CHANNEL_ETAG, 'is_archived': True} }}    expected_status=any
