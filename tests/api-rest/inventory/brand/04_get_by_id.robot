...
Get With Columns Succeeds
    ${resp}=    GET On Session    api    ${BRAND_API}/${BRAND_ID}
    ...    params=${{ {'fields': ['code', 'name', 'website']} }}
...
Get With Not Found Id Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${BRAND_API}/${NOT_FOUND_ID}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Get With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${BRAND_API}/not-existing-1234567890123    expected_status=any
