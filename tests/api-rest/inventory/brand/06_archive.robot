...
Archive Succeeds
    ...
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $BRAND_ETAG, 'is_archived': True} }}
...
Unarchive Succeeds
    ${resp}=    POST On Session    api    ${BRAND_API}/${BRAND_ID}/archived
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $BRAND_ETAG, 'is_archived': False} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${BRAND_ETAG}
    IF    $etag is not None    Set Global Variable    ${BRAND_ETAG}    ${etag}
...
Archive With Not Found Id Fails
    ...
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $BRAND_ETAG, 'is_archived': True} }}    expected_status=any
