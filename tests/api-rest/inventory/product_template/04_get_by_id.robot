...
Get With Columns Succeeds
    ${resp}=    GET On Session    api    ${PRODUCT_TEMPLATE_API}/${PRODUCT_TEMPLATE_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['name', 'status', 'category_id']} }}
...
Get With Edge Columns Succeeds
    ...
    ${resp}=    GET On Session    api    ${PRODUCT_TEMPLATE_API}/${PRODUCT_TEMPLATE_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['name', 'variants.sku']} }}
    Response Status Should Be    ${resp}    200
...
Get With Classification Edges Succeeds
    ...
    ${resp}=    GET On Session    api    ${PRODUCT_TEMPLATE_API}/${PRODUCT_TEMPLATE_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['name', 'product_type.code', 'category.code']} }}
    Response Status Should Be    ${resp}    200
...
Get With Not Found Id Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${PRODUCT_TEMPLATE_API}/${NOT_FOUND_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Get With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${PRODUCT_TEMPLATE_API}/not-existing-1234567890123
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
