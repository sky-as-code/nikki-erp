...
Update Succeeds
    ...
    ...    json=${{ {'org_id': $INV_ORG_ID, 'name': {'en-US': $name}, 'etag': $PRODUCT_ATTRIBUTE_ETAG} }}
...
Update Variant Creation Mode Succeeds
    ...
    ${resp}=    PATCH On Session    api    ${PRODUCT_ATTRIBUTE_API}/${PRODUCT_ATTRIBUTE_ID}
    ...    json=${{ {'org_id': $INV_ORG_ID, 'variant_creation_mode': 'dynamic', 'etag': $PRODUCT_ATTRIBUTE_ETAG} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${PRODUCT_ATTRIBUTE_ETAG}
    IF    $etag is not None    Set Global Variable    ${PRODUCT_ATTRIBUTE_ETAG}    ${etag}
...
Update With Invalid Variant Creation Mode Fails
    ...
    [Tags]    negative
    ${resp}=    PATCH On Session    api    ${PRODUCT_ATTRIBUTE_API}/${PRODUCT_ATTRIBUTE_ID}
    ...    json=${{ {'org_id': $INV_ORG_ID, 'variant_creation_mode': 'sometimes', 'etag': $PRODUCT_ATTRIBUTE_ETAG} }}
    ...    expected_status=any
...
Update With Invalid Id Format Fails
    ...
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $PRODUCT_ATTRIBUTE_ETAG} }}    expected_status=any
