*** Settings ***
Documentation     Archiving the Product Attribute under test, rotating the saved etag. The
...               attribute is unarchived again so the later suites see it live.
Resource          resources/inventory.resource
Suite Setup       Run Keywords    Create Authorized API Session    AND    Ensure Product Attribute Under Test
Test Tags         inventory    product_attribute    archive


*** Test Cases ***
Archive Succeeds
    ${resp}=    POST On Session    api    ${PRODUCT_ATTRIBUTE_API}/${PRODUCT_ATTRIBUTE_ID}/archived
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $PRODUCT_ATTRIBUTE_ETAG, 'is_archived': True} }}
...
Archived Attribute Is Still Readable
    [Documentation]    Archiving is visibility, not deletion: an existing template attribute
    ...    junction may still point at this attribute, and its detail page must be able to
    ...    name it.
...
Unarchive Succeeds
    ${resp}=    POST On Session    api    ${PRODUCT_ATTRIBUTE_API}/${PRODUCT_ATTRIBUTE_ID}/archived
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $PRODUCT_ATTRIBUTE_ETAG, 'is_archived': False} }}
    ${etag}=    Response Should Be Update Success    ${resp}    count=1    previous_etag=${PRODUCT_ATTRIBUTE_ETAG}
    IF    $etag is not None    Set Global Variable    ${PRODUCT_ATTRIBUTE_ETAG}    ${etag}

Archive With Not Found Id Fails
    [Tags]    negative
    ${resp}=    POST On Session    api    ${PRODUCT_ATTRIBUTE_API}/${NOT_FOUND_ID}/archived
    ...    json=${{ {'org_id': $INV_ORG_ID, 'etag': $PRODUCT_ATTRIBUTE_ETAG, 'is_archived': True} }}    expected_status=any
