...
Get With Columns Succeeds
    ${resp}=    GET On Session    api    ${ATTRIBUTE_VALUE_API}/${ATTRIBUTE_VALUE_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['code', 'name', 'price_extra']} }}
...
Get With Edge Columns Succeeds
    ...
    ...    attribute through.
    ${resp}=    GET On Session    api    ${ATTRIBUTE_VALUE_API}/${ATTRIBUTE_VALUE_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['code', 'attribute.code']} }}
    Response Status Should Be    ${resp}    200
...
Get With Not Found Id Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${ATTRIBUTE_VALUE_API}/${NOT_FOUND_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Get With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${ATTRIBUTE_VALUE_API}/not-existing-1234567890123
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
