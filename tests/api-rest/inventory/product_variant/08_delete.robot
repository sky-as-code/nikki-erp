...
Deleting A Variant Frees Its Combination
    ...
    DELETE On Session    api    ${PRODUCT_VARIANT_API}/${second_id}
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
...
Delete With Not Found Id Fails
    [Tags]    negative
    ${resp}=    DELETE On Session    api    ${PRODUCT_VARIANT_API}/${NOT_FOUND_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Delete With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    DELETE On Session    api    ${PRODUCT_VARIANT_API}/not-existing-1234567890123
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
