*** Settings ***
Documentation     Deleting the Stock Location under test — always the LAST suite, doubling
...
Test Tags         inventory    stock_location    delete
...
Delete With Referencing Child Fails
    ...
    [Tags]    negative
    ${child}=    Get Variable Value    ${CHILD_STOCK_LOCATION_ID}    ${EMPTY}
    IF    not $child    Skip    No child location was created by 02_create.robot
    ${resp}=    DELETE On Session    api    ${STOCK_LOCATION_API}/${STOCK_LOCATION_ID}    expected_status=any
...
Delete Child Succeeds
    ${child}=    Get Variable Value    ${CHILD_STOCK_LOCATION_ID}    ${EMPTY}
    IF    not $child    Skip    No child location was created by 02_create.robot
    ${resp}=    DELETE On Session    api    ${STOCK_LOCATION_API}/${child}
    Response Should Be Delete Success    ${resp}    count=1
    Set Global Variable    ${CHILD_STOCK_LOCATION_ID}    ${EMPTY}

Delete Succeeds
    ${resp}=    DELETE On Session    api    ${STOCK_LOCATION_API}/${STOCK_LOCATION_ID}
    Response Should Be Delete Success    ${resp}    count=1
...
Delete Again With Same Id Fails
    ...
    Set Global Variable    ${STOCK_LOCATION_ID}    ${EMPTY}
    Set Global Variable    ${STOCK_LOCATION_ETAG}    ${EMPTY}

Delete With Not Found Id Fails
    [Tags]    negative
    ${resp}=    DELETE On Session    api    ${STOCK_LOCATION_API}/${NOT_FOUND_ID}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Delete With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    DELETE On Session    api    ${STOCK_LOCATION_API}/not-existing-1234567890123    expected_status=any
