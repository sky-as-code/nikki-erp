*** Settings ***
Documentation     Creating Stock Locations. The first test saves the location under test
...               (${STOCK_LOCATION_ID}/${STOCK_LOCATION_ETAG}) consumed by the later
...
Test Tags         inventory    stock_location    create


*** Test Cases ***
Create With Required Fields Succeeds
    ${name}=    Unique Display Name    Robot Stock Location
    ${code}=    Unique Code    stkloc
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
    ...    json=${{ {'code': $code, 'name': {'en-US': $name}, 'location_type': 'internal', 'org_id': $INV_ORG_ID} }}
    ${id}    ${etag}=    Response Should Be Create Success    ${resp}
    Set Global Variable    ${STOCK_LOCATION_ID}    ${id}
    Set Global Variable    ${STOCK_LOCATION_ETAG}    ${etag}
    Set Global Variable    ${STOCK_LOCATION_CODE}    ${code}
...
Create With Parent Location Succeeds
    ...
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
    ...    json=${{ {'code': $code, 'name': {'en-US': $name}, 'location_type': 'internal', 'parent_location_id': $STOCK_LOCATION_ID, 'org_id': $INV_ORG_ID} }}
    ${id}    ${etag}=    Response Should Be Create Success    ${resp}
    ${resp}=    GET On Session    api    ${STOCK_LOCATION_API}/${id}
    ${item}=    Item Should Match Schema    ${resp}    ${INVENTORY_SCHEMA_DIR}/stock_location.json    200
    Should Be Equal    ${item}[parent_location_id]    ${STOCK_LOCATION_ID}
    Set Global Variable    ${CHILD_STOCK_LOCATION_ID}    ${id}
...
Create Each Virtual Location Type Succeeds
    ...
    FOR    ${type}    IN    customer    supplier    inventory_loss    scrap    transit
        ${name}=    Unique Display Name    Robot ${type} Location
        ${code}=    Unique Code    ${type}
        ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
        ...    json=${{ {'code': $code, 'name': {'en-US': $name}, 'location_type': $type, 'org_id': $INV_ORG_ID} }}
        ${id}    ${etag}=    Response Should Be Create Success    ${resp}
        DELETE On Session    api    ${STOCK_LOCATION_API}/${id}    expected_status=any
    END

Create Defaults Location Type To Internal
    [Documentation]    The schema defaults location_type to `internal`, the only type that
    ...
    ${resp}=    GET On Session    api    ${STOCK_LOCATION_API}/${id}
    ${item}=    Item Should Match Schema    ${resp}    ${INVENTORY_SCHEMA_DIR}/stock_location.json    200
    Should Be Equal    ${item}[location_type]    internal
    DELETE On Session    api    ${STOCK_LOCATION_API}/${id}    expected_status=any
...
Create With Unknown Location Type Fails
    ...
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
    ...    json=${{ {'code': $code, 'name': {'en-US': $name}, 'location_type': 'warehouse', 'org_id': $INV_ORG_ID} }}
    ...    expected_status=any
    Should Not Be Equal As Integers    ${resp.status_code}    201
    ...    msg=A location_type outside the declared enum must not be accepted

Create With Duplicate Code And Org Fails
    [Documentation]    composite_uniques on stock_location.json is ["code", "org_id"]: the
    ...    same code is only ambiguous within one org.
    [Tags]    negative
    ${name}=    Unique Display Name    Robot Duplicate Location
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
    ...    json=${{ {'code': $STOCK_LOCATION_CODE, 'name': {'en-US': $name}, 'location_type': 'internal', 'org_id': $INV_ORG_ID} }}
    ...    expected_status=any
    Response Should Be Duplicate Values Error    ${resp}    code    org_id

Create With Missing Required Fields Fails
    [Tags]    negative
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}    json=${{ {} }}    expected_status=any
...
Create With Archived Flag Fails
    ...
    ${resp}=    POST On Session    api    ${STOCK_LOCATION_API}
    ...    json=${{ {'code': $code, 'name': {'en-US': $name}, 'location_type': 'internal', 'org_id': $INV_ORG_ID, 'is_archived': True} }}
    ...    expected_status=any
    Should Not Be Equal As Integers    ${resp.status_code}    201
