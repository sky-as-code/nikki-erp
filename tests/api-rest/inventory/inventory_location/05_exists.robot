*** Settings ***
Documentation     Existence checks over Inventory Locations.
Resource          resources/inventory.resource
Suite Setup       Run Keywords    Create Authorized API Session    AND    Ensure Inventory Location Under Test
Test Tags         inventory    inventory_location    exists


*** Test Cases ***
Exists With One Id Succeeds
    ${resp}=    POST On Session    api    ${INVENTORY_LOCATION_API}/exists
    ...    json=${{ {'org_id': $INV_ORG_ID, 'ids': [$INVENTORY_LOCATION_ID]} }}
...
Exists With Mixed Ids Succeeds
    ${resp}=    POST On Session    api    ${INVENTORY_LOCATION_API}/exists
    ...    json=${{ {'org_id': $INV_ORG_ID, 'ids': [$INVENTORY_LOCATION_ID, $NOT_FOUND_ID]} }}
    Response Should Be Exists Success    ${resp}    existing=1    not_existing=1

Exists With Empty Ids Fails
    [Tags]    negative
    ${resp}=    POST On Session    api    ${INVENTORY_LOCATION_API}/exists
    ...    json=${{ {'org_id': $INV_ORG_ID, 'ids': []} }}    expected_status=any
