...
Get With Columns Succeeds
    ${resp}=    GET On Session    api    ${PRODUCT_CATEGORY_API}/${PRODUCT_CATEGORY_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID, 'fields': ['code', 'name', 'parent_category_id']} }}
...
Get With Not Found Id Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${PRODUCT_CATEGORY_API}/${NOT_FOUND_ID}
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
    Response Should Be Not Found Error    ${resp}

Get With Invalid Id Format Fails
    [Tags]    negative
    ${resp}=    GET On Session    api    ${PRODUCT_CATEGORY_API}/not-existing-1234567890123
    ...    params=${{ {'org_id': $INV_ORG_ID} }}    expected_status=any
